...
#alternatively, instead of trying to locally re-parse the numbers next to gears
# we can put them back into a grid for querying
igrid = [[0]*len(ngrid[0]) for _ in range(len(ngrid))]
for b in range(len(ngrid)):
    row_ints = ints(ngrid[b])
    idx = 0
    last_digit = False
    for a in range(len(ngrid[0])):
        if ngrid[b][a].isdigit():
            igrid[b][a] = row_ints[idx]
            last_digit = True
        else:
            if last_digit:
                last_digit = False
                idx += 1
def nums_at_2(a,b):
    # assume numbers are distinct
    ns = set(igrid[b-1][a-1:a+2] + igrid[b][a-1:a+2] + igrid[b+1][a-1:a+2]) - {0}
    if len(ns)==2:
        return ns.pop()*ns.pop()
    else:
        return 0

print(sum([res for b in range(len(lines)) for a in range(len(lines[0]))
         if lines[b][a]=='*' for res in [nums_at_2(a,b)] if res]))
