...
i = lines.index('') # split on empty line
	...
	try:
		while not done:
			...
			if not cont: # no immune groups will do any more damage
				print "no immune group could do damage"
				done=1
			if DEBUG:
				print "\n"
			...
			# if no immune unit can kill an infect unit
			if not done:
				stalemate = not any( naive_damage(g1,g2)//g2[3] >=1
						for g1 in all_groups
						for g2 in all_groups  if g1[-1]!=g2[-1])
				if stalemate:
					print "stalemate with boost",boost
					done=1 # no groups can damage each other
	except:
		print 'boost:',boost,'groups:',len(all_groups)
		print 'targeted:',targeted,'targets:',targets
		print groups
		raise
	if boost==0:
		print "part 1",sum(g[2] for g in all_groups) # total units
